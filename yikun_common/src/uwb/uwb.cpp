...
UWB::UWB(const AnchorArray anchorArray,const std::string port,
         const unsigned int baudrate,const unsigned int timeout)
  : port_(port),baudrate_(baudrate),timeout_(timeout)
{
  //A0
  anchorArray_[0].x = anchorArray.A0[0];
  anchorArray_[0].y = anchorArray.A0[1];
  anchorArray_[0].z = anchorArray.A0[2];
  //A1
  anchorArray_[1].x = anchorArray.A1[0];
  anchorArray_[1].y = anchorArray.A1[1];
  anchorArray_[1].z = anchorArray.A1[2];
  //A2
  anchorArray_[2].x = anchorArray.A2[0];
  anchorArray_[2].y = anchorArray.A2[1];
  anchorArray_[2].z = anchorArray.A2[2];
  //A3
  anchorArray_[3].x = anchorArray.A3[0];
  anchorArray_[3].y = anchorArray.A3[1];
  anchorArray_[3].z = anchorArray.A3[2];
}
...
bool UWB::connect()
{
  try
  {
    serial_.setPort(port_);	//串口
    serial_.setBaudrate(baudrate_);//波特率115200
  ...
  {
    ROS_ERROR("Unable to open UWB port. %s",e.what());
    return false;
  }
  //check again
  if (serial_.isOpen()) {
    ROS_INFO("UWB serial port '%s' initialized.",port_.data());
    return true;
  } else {
    return false;
  }
...
bool UWB::communicate(std::string &info)
{
  //MID MASK RANGE0 RANGE1 RANGE2 RANGE3 NRANGES RSEQ DEBUG aT:A
  if(serial_.available()) {
    info = serial_.read(serial_.available());
    std::cout<<"recv: "<<info.data()<<std::endl;
  } else {
    ROS_DEBUG_STREAM("serial isn't available");
  }
}
bool UWB::getNewData(std::vector<UWBInfo>& info)
{
  //MID MASK RANGE0 RANGE1 RANGE2 RANGE3 NRANGES RSEQ DEBUG aT:A
  if(serial_.available()) {
    std::string stream = serial_.read(serial_.available());	//读串口
    std::vector<std::string> streams;
    boost::split(streams,stream,boost::is_any_of("\n"));
    for(auto str : streams) {
      std::vector<std::string> msg;
      boost::split(msg,str,boost::is_any_of(" "));
        if(msg[0] == "mr" && msg[1] == "0f")//MASK=0f，数据有效
        {
          int range[4];
          for(int i = 0;i < 4;i++){
            range[i] = strtol(msg[i+2].c_str(),NULL,16);
          }
          std::vector<std::string> tag;
          boost::split(tag,msg[9],boost::is_any_of(":"));
          std::string tagstr = tag[0].substr(1);
          UWBInfo tag_info;
          tag_info.tag_id = atoi(tagstr.c_str());
          this->GetLocation(&tag_info.position,1,&anchorArray_[0],&range[0]);
          ROS_DEBUG("tag%d: %.4f,%.4f,%.4f",tag_info.tag_id,
                   tag_info.position.x,tag_info.position.y,tag_info.position.z);
...
vec3d UWB::vdiff(const vec3d vector1, const vec3d vector2)
{
  vec3d v;
...
vec3d UWB::vsum(const vec3d vector1, const vec3d vector2)
{
  vec3d v;
  v.x = vector1.x + vector2.x;
  v.y = vector1.y + vector2.y;
...
vec3d UWB::vmul(const vec3d vector, const double n)
{
  vec3d v;
  v.x = vector.x * n;
  v.y = vector.y * n;
...
vec3d UWB::vdiv(const vec3d vector, const double n)
{
  vec3d v;
  v.x = vector.x / n;
  v.y = vector.y / n;
...
double UWB::vdist(const vec3d v1, const vec3d v2)
{
  double xd = v1.x - v2.x;
  double yd = v1.y - v2.y;
  double zd = v1.z - v2.z;
...
double UWB::vnorm(const vec3d vector)
{
  return sqrt(vector.x * vector.x + vector.y * vector.y + vector.z * vector.z);
}

double UWB::dot(const vec3d vector1, const vec3d vector2)
{
  return vector1.x * vector2.x + vector1.y * vector2.y + vector1.z * vector2.z;
}

vec3d UWB::cross(const vec3d vector1, const vec3d vector2)
{
  vec3d v;
...
double UWB::gdoprate(const vec3d tag, const vec3d p1, const vec3d p2, const vec3d p3)
{
  vec3d ex, t1, t2, t3;
...
int UWB::sphereline(const vec3d p1, const vec3d p2, const vec3d sc, double r, double *const mu1, double *const mu2)
{
   double a,b,c;
   double bb4ac;
   vec3d dp;
...
int UWB::trilateration(vec3d *const result1,
          vec3d *const result2,
...
int UWB::deca_3dlocate (	vec3d	*const solution1,
...
int UWB::GetLocation(vec3d *best_solution, int use4thAnchor, vec3d* anchorArray, int *distanceArray)
{
